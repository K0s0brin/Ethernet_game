...
// APB bus interface
static inline void hal_uart_init(USART_TypeDef *uart,
                                  uint16_t tx, uint8_t tx_af,
                                  uint16_t rx, uint8_t rx_af,
                                  unsigned long baud) {
  uint32_t freq = 0;        // Bus frequency. UART1 is on APB2, rest on APB1

  if (uart == USART1) freq = HAL_RCC_GetPCLK2Freq(), RCC->APB2ENR |= RCC_APB2ENR_USART1EN;
  else if (uart == USART3) freq = HAL_RCC_GetPCLK1Freq(), RCC->APB1LENR |= BIT(18);
  hal_gpio_init(tx, HAL_GPIO_MODE_AF, HAL_GPIO_OTYPE_PUSH_PULL, HAL_GPIO_SPEED_HIGH, 0, tx_af);
  hal_gpio_init(rx, HAL_GPIO_MODE_AF, HAL_GPIO_OTYPE_PUSH_PULL, HAL_GPIO_SPEED_HIGH, 0, rx_af);
